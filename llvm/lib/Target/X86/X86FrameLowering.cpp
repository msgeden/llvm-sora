...
/// if SORA is enabled
bool X86FrameLowering::hasSORA(const MachineFunction &MF) const {
    return MF.hasSORA();
}
...
void X86FrameLowering::emitAES128EncryptionRounds(MachineBasicBlock &MBB,
    ...
    BuildMI(MBB,MBBI,DL,TII.get(X86::AESENCrr),IVector).addReg(IVector).addReg(X86::XMM4);
    //    BuildMI(MBB,MBBI,DL,TII.get(X86::AESENCrr),IVector).addReg(IVector).addReg(X86::XMM5);
    //    BuildMI(MBB,MBBI,DL,TII.get(X86::AESENCrr),IVector).addReg(IVector).addReg(X86::XMM6);
    //    BuildMI(MBB,MBBI,DL,TII.get(X86::AESENCrr),IVector).addReg(IVector).addReg(X86::XMM7);
    //    BuildMI(MBB,MBBI,DL,TII.get(X86::AESENCrr),IVector).addReg(IVector).addReg(X86::XMM8);
    //    BuildMI(MBB,MBBI,DL,TII.get(X86::AESENCrr),IVector).addReg(IVector).addReg(X86::XMM9);
    //    BuildMI(MBB,MBBI,DL,TII.get(X86::AESENCrr),IVector).addReg(IVector).addReg(X86::XMM10);
    //    BuildMI(MBB,MBBI,DL,TII.get(X86::AESENCrr),IVector).addReg(IVector).addReg(X86::XMM11);
    //    BuildMI(MBB,MBBI,DL,TII.get(X86::AESENCrr),IVector).addReg(IVector).addReg(X86::XMM12);
    //    BuildMI(MBB,MBBI,DL,TII.get(X86::AESENCrr),IVector).addReg(IVector).addReg(X86::XMM13);
    //    BuildMI(MBB,MBBI,DL,TII.get(X86::AESENCrr),IVector).addReg(IVector).addReg(X86::XMM14);
    BuildMI(MBB,MBBI,DL,TII.get(X86::AESENCLASTrr),IVector).addReg(IVector).addReg(X86::XMM15);
    return;
}
...
void X86FrameLowering::emitPrologue(MachineFunction &MF,
    ...
    if (HasFP) {
        ...
        //Number of required XMM registers for return address, message length, RBP (base pointer), RBX (tag), pushed GPRs
        unsigned MACBlockCount=(MF.getFrameInfo().getCalleeSavedInfo().size()+1+2+2)/2;
        if (HasSORA){
            //CBC-MAC first round for prologue
            //Prepare zero vector
            BuildMI(MBB,MBBI,DL,TII.get(X86::PXORrr),X86::XMM2).addReg(X86::XMM2).addReg(X86::XMM2);
            //Prepend message length
            BuildMI(MBB,MBBI,DL,TII.get(X86::MOV64ri)).addReg(X86::R11).addImm(MACBlockCount);
            BuildMI(MBB,MBBI,DL,TII.get(X86::MOV64toPQIrr)).addReg(X86::XMM3).addReg(X86::R11);
            //Shift left
            BuildMI(MBB,MBBI,DL,TII.get(X86::PSLLDQri),X86::XMM3).addReg(X86::XMM3).addImm(8);
            //Include return address
            //FIXME: Check Return Address Offset
            addRegOffset(BuildMI(MBB, MBBI, DL, TII.get(X86::MOV64toPQIrm), X86::XMM3),
                         X86::RSP, false, +8);
            //Perform AES rounds
            emitAES128EncryptionRounds(MBB, MBBI, DL, X86::XMM2, X86::XMM3);
            //NOOP identifier for instrumentation
        ...
        //BEGINNING OF SORA
        //Following prologue instrumentation for base pointer and caller's MAC tag on RBX
        if (HasSORA){
            //Push caller's MAC tag (RBX)
            BuildMI(MBB, MBBI, DL, TII.get(Is64Bit ? X86::PUSH64r : X86::PUSH32r))
            .addReg(Is64Bit ? X86::RBX : X86::EBX, RegState::Kill)
            .setMIFlag(MachineInstr::FrameSetup);
            ...
            if (NeedsDwarfCFI) {
                ...
                assert(StackSize);
                BuildCFI(MBB, MBBI, DL,
                         MCCFIInstruction::createDefCfaOffset(nullptr, 3 * stackGrowth));
                
                // Change the rule for the FramePtr to be an "offset" rule.
                unsigned DwarfFramePtr = TRI->getDwarfRegNum(MachineFramePtr, true);
                BuildCFI(MBB, MBBI, DL, MCCFIInstruction::createOffset(
                                                                       nullptr, DwarfFramePtr, 3 * stackGrowth));
            }
            
            //Include base pointer
            BuildMI(MBB,MBBI,DL,TII.get(X86::MOV64toPQIrr)).addReg(X86::XMM3).addReg(MachineFramePtr);
            //Shift left
            BuildMI(MBB,MBBI,DL,TII.get(X86::PSLLDQri),X86::XMM3).addReg(X86::XMM3).addImm(8);
            //Include callee's MAC register
            BuildMI(MBB,MBBI,DL,TII.get(X86::MOV64toPQIrr)).addReg(X86::XMM3).addReg(X86::RBX);
            //Perform AES rounds
            emitAES128EncryptionRounds(MBB, MBBI, DL, X86::XMM2, X86::XMM3);
            //Truncate the tag and keep on RBX register if no GPR push
            if (MACBlockCount==2)
                BuildMI(MBB,MBBI,DL,TII.get(X86::MOV64toPQIrr)).addReg(X86::RBX).addReg(X86::XMM2);
        }//END OF SORA
...
void X86FrameLowering::emitEpilogue(MachineFunction &MF,
    ...
    if (HasFP) {
        ...
        if (HasSORA){
            ...
            if (MFI.getCalleeSavedInfo().empty()){
                ...
                //CBC-MAC round for epilogue beginning in case of no other register spills exist
                //Prepare zero vector
                BuildMI(MBB,MBBI,DL,TII.get(X86::PXORrr),X86::XMM2).addReg(X86::XMM2).addReg(X86::XMM2);
                //Prepend message length
                BuildMI(MBB,MBBI,DL,TII.get(X86::MOV64ri)).addReg(X86::R11).addImm(MACBlockCount);
                ...
                //Include return address
                BuildMI(MBB,MBBI,DL,TII.get(X86::PSLLDQri),X86::XMM3).addReg(X86::XMM3).addImm(8);
                //At this point RBP should the caller's RSP set at the prologue
                //FIXME: Check Return Address Offset
                addRegOffset(BuildMI(MBB, MBBI, DL, TII.get(X86::MOV64toPQIrm), X86::XMM3),
                             X86::RBP, false, +8);
                //Perform AES rounds
                ...
                //NOOP identifiers for instrumentation
                //BuildMI(MBB, MBBI, DL, TII.get(X86::NOOP));
                
                //Retrieve RBP and RBX (MAC) registers with the push-given order using MOV operations instead of POP operations
                
                //POP EBP using MOV
                addRegOffset(BuildMI(MBB, MBBI, DL, TII.get(STI.is64Bit() ? X86::MOV64rm : X86::MOV32rm), STI.is64Bit() ? X86::RBP : X86::EBP),
                             STI.is64Bit() ? X86::RSP : X86::ESP, false, STI.is64Bit() ? 8 : 4).setMIFlag(MachineInstr::FrameDestroy);
                
                //POP RBX (tag) using MOV
                addRegOffset(BuildMI(MBB, MBBI, DL, TII.get(STI.is64Bit() ? X86::MOV64rm : X86::MOV32rm), STI.is64Bit() ? X86::RBX : X86::EBX),
                             STI.is64Bit() ? X86::RSP : X86::ESP, false, 0).setMIFlag(MachineInstr::FrameDestroy);
                
                //Adjust stack pointer accordingly
                BuildMI(MBB,MBBI,DL, TII.get(STI.is64Bit() ? X86::ADD64ri8 : X86::ADD32ri8), STI.is64Bit() ? X86::RSP : X86::ESP).addReg(STI.is64Bit() ? X86::RSP : X86::ESP).addImm(STI.is64Bit() ? 16: 8).setMIFlag(MachineInstr::FrameDestroy);;
                
                //CBC-MAC round for RBP and RBX values
                BuildMI(MBB,MBBI,DL,TII.get(X86::MOV64toPQIrr)).addReg(X86::XMM3).addReg(X86::RBP);
                BuildMI(MBB,MBBI,DL,TII.get(X86::PSLLDQri),X86::XMM3).addReg(X86::XMM3).addImm(8);
                BuildMI(MBB,MBBI,DL,TII.get(X86::MOV64toPQIrr)).addReg(X86::XMM3).addReg(X86::RBX);
                //Perform AES rounds
                emitAES128EncryptionRounds(MBB, MBBI, DL, X86::XMM2, X86::XMM3);
                ...
                //Truncate MAC tag
                BuildMI(MBB,MBBI,DL,TII.get(X86::MOV64toPQIrr)).addReg(X86::R11).addReg(X86::XMM2);
                //Compare truncated (64-bit) MAC tags of the epilogue (r11) with the prologue (rbx)
                BuildMI(MBB,MBBI,DL,TII.get(X86::CMP64rr)).addReg(X86::R11).addReg(X86::RBX);
                
                // Jump exit function if MAC tags do not match
                //MachineBasicBlock &EntryBB=MF.front();
                //entryMBB->splice(entryMBB->end(), &MBB, MBB.begin(), MBBI);
                //BuildMI(MBB, MBBI, DL, TII.get(X86::JCC_1)).addMBB(&MBB).addImm(X86::COND_NE).setMIFlag(MachineInstr::FrameDestroy);
                //BuildMI(MBB,MBBI,DL,TII.get(X86::JECXZ)).addBlockAddress(BlockAddress);
    ...
    if ((TRI->needsStackRealignment(MF) || MFI.hasVarSizedObjects()) &&
        ...
        // Added for SORA
        if (!(HasSORA && MFI.getCalleeSavedInfo().empty()))
            emitSPUpdate(MBB, MBBI, DL, NumBytes, /*InEpilogue=*/true);
...
bool X86FrameLowering::spillCalleeSavedRegisters(
    ...
    //Added for SORA
    bool HasSORA=MBB.getParent()->hasSORA();
    int GPRPushCount=0;
    ...
    for (unsigned i = CSI.size(); i != 0; --i) {
        ...
        //FIXME: (SORA) define RBX/EBX as reserved register for MAC tags in the calling convention
        BuildMI(MBB, MI, DL, TII.get(Opc)).addReg(Reg, getKillRegState(CanKill))
        .setMIFlag(MachineInstr::FrameSetup);
        
        
        //BEGINNING OF SORA
        //Prologue instrumentation for GPRs pushes
        //Number of required XMM registers for return address, message length, RBP (base pointer), RBX (tag), pushed GPRs
        unsigned MACBlockCount=(MF.getFrameInfo().getCalleeSavedInfo().size()+1+2+2)/2;
        if (HasSORA){
            //Keeps track of pushed GPRs to adjust XMM message blocks
            GPRPushCount++;
            //Decides on when to truncate the MAC tag
            bool IsLastPush=((i-1)==0);
            if (MACBlockCount>2){
                if (GPRPushCount%2==1){
                    BuildMI(MBB,MI,DL,TII.get(X86::MOV64toPQIrr)).addReg(X86::XMM3).addReg(Reg);
                    BuildMI(MBB,MI,DL,TII.get(X86::PSLLDQri),X86::XMM3).addReg(X86::XMM3).addImm(8);
                    if (IsLastPush){
                        emitAES128EncryptionRounds(MBB, MI, DL, X86::XMM2, X86::XMM3);
                    }
                }
                if (GPRPushCount%2==0){
                    BuildMI(MBB,MI,DL,TII.get(X86::MOV64toPQIrr)).addReg(X86::XMM3).addReg(Reg);
            ...
            }//Truncate the tag and keep on RBX register if there is no GPR push
            if (IsLastPush){
                BuildMI(MBB,MI,DL,TII.get(X86::MOV64toPQIrr)).addReg(X86::RBX).addReg(X86::XMM2);
            }
        }//END OF SORA
    }
...
bool X86FrameLowering::restoreCalleeSavedRegisters(MachineBasicBlock &MBB,
    ...
    bool HasFP = hasFP(*MF);
    bool HasSORA=MF->hasSORA();
    //Operation and register types based address architecture
    unsigned OpcMov = STI.is64Bit() ? X86::MOV64rm : X86::MOV32rm;
    unsigned OpcAdd = STI.is64Bit() ? X86::ADD64ri8 : X86::ADD32ri8;
    unsigned RegSP = STI.is64Bit() ? X86::RSP : X86::ESP;
    unsigned RegBX = STI.is64Bit() ? X86::RBX : X86::EBX;
    unsigned RegBP = STI.is64Bit() ? X86::RBP : X86::EBP;
    unsigned MACBlockCount=(MF->getFrameInfo().getCalleeSavedInfo().size()+1+2+2)/2;
    //Size of a GPR in bytes
    unsigned RegSize = STI.is64Bit() ? 8 : 4;
    //Keeps track number of bytes for reverse register pops with MOV operations
    unsigned PoppedBytes=0;
    //Keeps track of GPRs popped back
    int GPRPopCount=2; //2 for BP and BX
    int GPRCount=2;
    int GPRi=2;
    for (unsigned i = CSI.size(); i>0 ; i--) {
    ...
    if (HasFP){
        //First register stores of base of pointer (RBP) and tag values (RBX) registers
        if (HasSORA){
            BuildMI(MBB,MI,DL,TII.get(X86::PXORrr),X86::XMM2).addReg(X86::XMM2).addReg(X86::XMM2);
            //Prepend message length
            BuildMI(MBB,MI,DL,TII.get(X86::MOV64ri)).addReg(X86::R11).addImm(MACBlockCount);
            BuildMI(MBB,MI,DL,TII.get(X86::MOV64toPQIrr)).addReg(X86::XMM3).addReg(X86::R11);
            //Shift left
            BuildMI(MBB,MI,DL,TII.get(X86::PSLLDQri),X86::XMM3).addReg(X86::XMM3).addImm(8);
            //Include return address
            //FIXME: Check Return Address Offset
            addRegOffset(BuildMI(MBB, MI, DL, TII.get(X86::MOV64toPQIrm), X86::XMM3),
                         X86::RBP, false, +8);
            //Perform AES rounds
            emitAES128EncryptionRounds(MBB, MI, DL, X86::XMM2, X86::XMM3);
            
            addRegOffset(BuildMI(MBB, MI, DL, TII.get(OpcMov), RegBP),
                         RegSP, false, (GPRCount-1)*RegSize).setMIFlag(MachineInstr::FrameDestroy);
            PoppedBytes+=RegSize;
            
            addRegOffset(BuildMI(MBB, MI, DL, TII.get(OpcMov), RegBX),
                         RegSP, false, (GPRCount-2)*RegSize).setMIFlag(MachineInstr::FrameDestroy);
            PoppedBytes+=RegSize;
            
            //Include base pointer
            BuildMI(MBB,MI,DL,TII.get(X86::MOV64toPQIrr)).addReg(X86::XMM3).addReg(RegBP);
            //Shift left
            BuildMI(MBB,MI,DL,TII.get(X86::PSLLDQri),X86::XMM3).addReg(X86::XMM3).addImm(8);
            //Include callee's MAC register
            BuildMI(MBB,MI,DL,TII.get(X86::MOV64toPQIrr)).addReg(X86::XMM3).addReg(X86::RBX);
            //Perform AES rounds
            emitAES128EncryptionRounds(MBB, MI, DL, X86::XMM2, X86::XMM3);
        }
    }//END OF SORA
    ...
    //BEGINNING OF SORA
    //Epilogue instrumentation in case there are GPR pushes
    if (HasSORA){
        //Iterate GPRs in the same order of prologue pushes
        for (unsigned i = CSI.size(); i>0 ; i--) {
            int
            unsigned Reg = CSI[i-1].getReg();
            ...
            GPRi++;
            //FIXME: Should RegKilled be false or true?
            addRegOffset(BuildMI(MBB, MI, DL, TII.get(OpcMov), Reg),
                         RegSP, false, (GPRCount-GPRi)*RegSize ).setMIFlag(MachineInstr::FrameDestroy);
            PoppedBytes+=RegSize;
            
            GPRPopCount++;
            //NOOP identifiers for instrumentation
            //BuildMI(MBB,MI,DL, TII.get(X86::NOOP));
            bool IsLastPop=((i-1)==0);
            if (MACBlockCount>2){
                if (GPRPopCount%2==1){
                    BuildMI(MBB,MI,DL,TII.get(X86::MOV64toPQIrr)).addReg(X86::XMM3).addReg(Reg);
                    BuildMI(MBB,MI,DL,TII.get(X86::PSLLDQri),X86::XMM3).addReg(X86::XMM3).addImm(8);
                ...
                //NOOP identifiers for instrumentatin
                //BuildMI(MBB, MI, DL, TII.get(X86::NOOP));
            }
            if (IsLastPop){
                //Truncate MAC tag
                BuildMI(MBB,MI,DL,TII.get(X86::MOV64toPQIrr)).addReg(X86::R11).addReg(X86::XMM2);
                //Adjust stack pointer accordingly
                BuildMI(MBB,MI,DL, TII.get(OpcAdd),RegSP).addReg(RegSP).addImm(RegSize*GPRCount).setMIFlag(MachineInstr::FrameDestroy);;
                
                //Compare truncated (64-bit) MAC tags of the epilogue (r11) with the prologue (rbx)
                BuildMI(MBB,MI,DL,TII.get(X86::CMP64rr)).addReg(X86::R11).addReg(X86::RBX);
